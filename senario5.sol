 ...
 * @notice ترکیبی از بهترین رویکردها:
 *         - حذف تیم ولت (کاهش گس)
 *         - استقرار دو مرحله‌ای
 *         - مدیریت با AccessControl
 */
contract PSC is ERC20, ReentrancyGuard, AccessControl {
    ...
    // ============================
    // WALLETS — قابل تغییر توسط ADMIN_ROLE
    // ============================
    ...
    event WalletsUpdated(address developer, address f1, address f2, address consultant, address offering, address distribution);
    event Initialized(address indexed by);

    // ============================
    // CONSTRUCTOR — حداقل تنظیمات
    // ============================
    constructor()
    ...
    // ============================
    // INITIALIZE — تکمیل تنظیمات
    // ============================
    function initialize(address[] calldata airdropRecipientsList) external onlyAdmin onlyUninitialized {
        // 1% — Team → developerWallet
        uint256 teamTotalAmount = (TOTAL_SUPPLY * TEAM_PERCENT) / 100;
        _transfer(address(this), developerWallet, teamTotalAmount);
        ...
        // ایردراپ
        if (airdropRecipientsList.length > 0 && airdropRecipientsList.length <= MAX_AIRDROP_RECIPIENTS) {
            uint256 eachAmount = AIRDROP_AMOUNT / airdropRecipientsList.length;
            ...
            for (uint256 i = 0; i < airdropRecipientsList.length; ) {
                address recipient = airdropRecipientsList[i];
            ...
            emit AirdropDistributed(airdropRecipientsList, eachAmount, AIRDROP_LOCK_DURATION, AIRDROP_VESTING_DURATION);
        }
    ...
    // ============================
    // UPDATE WALLETS — فقط توسط ADMIN_ROLE
    // ============================
    function setWallets(
        ...
        emit WalletsUpdated(_developerWallet, _founder1, _founder2, _consultantWallet, _publicOfferingWallet, _publicDistributionWallet);
    }

    // ============================
    // AIRDROP — فقط توسط EXECUTOR_ROLE (اگر هنوز توزیع نشده)
    // ============================
    function distributeAirdrop(address[] calldata recipients) external onlyExecutor nonReentrant {
    ...
    // ============================
    // ANNUAL RELEASE — فقط توسط EXECUTOR_ROLE
    // ============================
