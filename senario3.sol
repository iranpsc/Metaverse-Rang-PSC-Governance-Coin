 ...
 * @notice Two-phase deployment: minimal deploy + initialize
 * @dev Phase 1: deploy contract (no token distribution)
 *      Phase 2: owner calls initialize() to complete setup
 */
contract PSC is ERC20, ReentrancyGuard {
        ...
        ERC20("Paradaise Supply Chain", "PSC")
    {
        ...
        // فقط توکن‌ها را ضرب می‌کنیم، اما توزیع نمی‌کنیم
    ...
    // ============================
    // INITIALIZE — تکمیل تنظیمات پس از دیپلوی
    // ============================
    function initialize(address[] calldata _airdropRecipients) external onlyOwner onlyUninitialized {
        ...
        // توزیع ایردراپ
        // ════════════════════════════════════════════════════════════
        if (_airdropRecipients.length > 0 && _airdropRecipients.length <= MAX_AIRDROP_RECIPIENTS) {
            uint256 eachAmount = AIRDROP_AMOUNT / _airdropRecipients.length;
            ...
            for (uint256 i = 0; i < _airdropRecipients.length; ) {
                address recipient = _airdropRecipients[i];
                ...
                // ✅ اصلاح: استفاده از نام state (airdropRecipients) بدون تداخل
            ...
            emit AirdropDistributed(_airdropRecipients, eachAmount, AIRDROP_LOCK_DURATION, AIRDROP_VESTING_DURATION);
        }
